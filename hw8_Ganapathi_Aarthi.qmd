...
datasummary(support_partner * 
              (N + Median + Mean + Min + Max + Histogram) ~
...
Ensure support variables are being treated as ordinal
``` {r}
...
```
The model seems to fit decently, but there still seems to be some overestimation happening, particularly for partner support. 
...
The specified priors does not seem to improve the model fit at all. 
...
@tbl-summmary-table1 shows the posterior distributions of the interaction between age and sexual orientation with the Li, Ji, & Chen (2014)-informed priors.
